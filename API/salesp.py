...
# Sales by Month - BAR
st.subheader("Sales by Month - Bar Chart")

month_sales = df_filtered.groupby("Month")["Sales"].sum()

fig2, ax2 = plt.subplots()
ax2.bar(month_sales.index, month_sales.values)
st.pyplot(fig2)

# Sales by Month - PIE
st.subheader("Sales Contribution by Month - Pie Chart")

fig3, ax3 = plt.subplots()
ax3.pie(month_sales.values, labels=month_sales.index, autopct="%1.1f%%")
st.pyplot(fig3)
